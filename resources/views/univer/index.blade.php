...
<div class="col-md-12">
  ...
  <div id="tri" class="btn-group btn-group-sm">
    <a href="{!! url('univer/member') !!}" type="button" name="total" class="btn btn-default {{ classActiveOnlyPath('univer/member') }}">ทั้งหมด
      <span class="badge">{{  $counts['total'] }}</span>
    </a>
    @foreach ($roles as $role)
      <a href="{!! url('univer/member/sort/' . $role->slug) !!}" type="button" name="{!! $role->slug !!}" class="btn btn-default {{ classActiveOnlySegment(3, $role->slug) }}">{{ $role->title }}
	...
	<div class="table-responsive">
        ...
        @foreach ($users as $user)
      		<tr {!! !$user->seen? 'class="warning"' : '' !!}>
      			...
      			<td>
              <a href="{{url("univer/member/".$user->id)}}" class="btn btn-success"> รายละเอียด </a>
      			</td>
      			<td>
      				<a href="{{url("univer/member/".$user->id."/edit")}}" class="btn btn-warning"> แก้ไข </a>
      			</td>
      			<td>
      				<form action="{{url('univer/member/'.$user->id)}}" method="post" onsubmit="return(confirm('ลบข้อมูล'))">
      					{{ method_field('DELETE') }}
    ...
    <a href="{{url("univer/member/create")}}" class="btn btn-primary pull-left"> เพิ่มสมาชิก </a>
  ...
  <script>
    ...
    $(function() {
      ...
      $(document).on('change', ':checkbox', function() {
        ...
        $.ajax({
          url: '{!! url('univer/member/userseen') !!}' + '/' + this.value,
          type: 'PUT',
