          ...
          <div class="box">
            ...
            <form role="form" id="form_data" name="form_data">
              ...
              <div class="form-group" id="j">
                ...
                <?php elseif(Auth::user()->role->slug == 'Operator'): ?>
                <h5><?php echo e(Auth::user()->area->name); ?></h5>
                <input type="hidden" name="area_id" id="area_id" value="0">
                <?php else: ?>
                <select name="area_id" id="area_id" class="form-control select2" style="width:350px">
                    <option value="">--- เลือกพื้นที่ชุมชน ---</option>
                    <?php foreach($objarea as $key => $value): ?>
                        <option value="<?php echo e($key); ?>"><?php echo e($value); ?></option>
                    <?php endforeach; ?>
                </select>
                <?php endif; ?>
...
<script type="text/javascript">
    $(function(){
      ...
      $('body').delegate('.edit','click',function(){
        ...
        var id = $(this).data('id');
        $.ajax({
            url : '<?php echo url('user/expert'); ?>'+'/'+id+'/edit',
            type : "get",
            //asyncfalse
            data : {
              '_token': '<?php echo e(csrf_token()); ?>'
      ...
      $('body').delegate('.delete','click',function(){
        ...
        $('.btndetail').show();
        $.ajax({
            url : '<?php echo url('user/expert'); ?>'+'/'+id,
            type : "POST",
            //asyncfalse
            data : {
              '_method':'DELETE',
    ...
    function displaydata(){
      //alert(0);
      $.ajax({
        url : '<?php echo url('user/expert/create'); ?>',
        type : "get",
        //asyncfalse
        data : {},
        success : function(s)
  ...
  function callcenter(idarea){
    $.ajax({
        ...
        url : '<?php echo url('ajaxcallcenter'); ?>'+'/'+idarea,
        type: "GET",
        //asyncfalse
        //url : '<?php echo url('user/tagdetail'); ?>',
        //type : "get",
  ...
  function loadselect(id,idarea){
    //alert(id);
        $.ajax({
            url : '<?php echo url('ajaxarea_uni'); ?>'+'/'+id,
            type: "GET",
            dataType: "json",
            success:function(data) {
