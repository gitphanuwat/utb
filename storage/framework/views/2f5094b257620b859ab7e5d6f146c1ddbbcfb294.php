...
<div class="col-md-12">
  ...
  <div id="tri" class="btn-group btn-group-sm">
    <a href="<?php echo url('univer/member'); ?>" type="button" name="total" class="btn btn-default <?php echo e(classActiveOnlyPath('univer/member')); ?>">ทั้งหมด
      <span class="badge"><?php echo e($counts['total']); ?></span>
    </a>
    <?php foreach($roles as $role): ?>
      <a href="<?php echo url('univer/member/sort/' . $role->slug); ?>" type="button" name="<?php echo $role->slug; ?>" class="btn btn-default <?php echo e(classActiveOnlySegment(3, $role->slug)); ?>"><?php echo e($role->title); ?>
	...
	<div class="table-responsive">
        ...
        <?php foreach($users as $user): ?>
      		<tr <?php echo !$user->seen? 'class="warning"' : ''; ?>>
      			...
      			<td>
              <a href="<?php echo e(url("univer/member/".$user->id)); ?>" class="btn btn-success"> รายละเอียด </a>
      			</td>
      			<td>
      				<a href="<?php echo e(url("univer/member/".$user->id."/edit")); ?>" class="btn btn-warning"> แก้ไข </a>
      			</td>
      			<td>
      				<form action="<?php echo e(url('univer/member/'.$user->id)); ?>" method="post" onsubmit="return(confirm('ลบข้อมูล'))">
    ...
    <a href="<?php echo e(url("univer/member/create")); ?>" class="btn btn-primary pull-left"> เพิ่มสมาชิก </a>
  ...
  <script>
    ...
    $(function() {
      ...
      $('body').delegate('.btndetail','click',function(){
          ...
          $('#showdetail').html('ttt');
          $.ajax({
              url : '<?php echo url('univer/member'); ?>'+'/'+id,
              type : "GET",
              //asyncfalse
              data : {
      ...
      $(document).on('change', ':checkbox', function() {
        ...
        $.ajax({
          url: '<?php echo url('univer/member/userseen'); ?>' + '/' + this.value,
          type: 'PUT',
